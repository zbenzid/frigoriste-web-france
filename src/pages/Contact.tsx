  ...
  const validateForm = () => {
    ...
    if (needsAddress) {
      if (!formData.address.trim()) {
        errors.push('L\'adresse est requise pour ce type de demande');
      }
      if (!formData.postalCode.trim()) {
        errors.push('Le code postal est requis pour ce type de demande');
      }
      if (!formData.city.trim()) {
        errors.push('La ville est requise pour ce type de demande');
      }
    }
    ...
    if (!formData.gdprConsent) {
      errors.push('Vous devez accepter notre politique de confidentialité');
    }
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                      <label htmlFor="message" className="block text-sm font-medium mb-1">
                        Message détaillé *
                        <span className="text-xs text-gray-500 ml-1">(minimum 10 caractères)</span>
                      </label>
                      <Textarea 
                        id="message" 
                        name="message" 
                      ...
                      <div className="flex justify-between items-center mt-1">
                        ...
                        <span className={`text-xs ${
                          messageLength === 0 ? 'text-gray-400' : 
                          isMessageValid ? 'text-green-600' : 'text-red-500'
                        }`}>
                          {messageLength}/10 caractères minimum
                        </span>
                      </div>
                    </div>
                    ...
                    <Button 
                      ...
                      disabled={isSubmitting || !isMessageValid || messageError !== ''}
                    >
                      {isSubmitting ? (
                        <>
